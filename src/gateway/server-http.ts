...
function isValidWebSocketOrigin(req: IncomingMessage): boolean {
  ...
  try {
    ...
    if (loopbackHosts.includes(originUrl.hostname)) {
      return true;
    }

    // Allow Tailscale domains
    if (originUrl.hostname.endsWith(".ts.net")) {
      return true;
    }

    // Allow internal moltbot domains
    if (originUrl.hostname.endsWith(".moltbot.internal")) {
      return true;
    }
    if (originUrl.hostname === "moltbot.internal") {
      return true;
    }
