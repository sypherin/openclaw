...
function parseGatewayConnectInfo(params: {
  url: string;
  auth: ResolveAuthResult;
  urlSource?: string;
}): GatewayConnectInfo | null {
  try {
    ...
    const port = portRaw ? Number.parseInt(portRaw, 10) : useTLS ? 443 : 80;
    if (!Number.isFinite(port) || port <= 0 || port > 65535) {
      return null;
    }
    const authLabel = params.auth.label ?? "auth";
    return {
      url: params.url,
      host,
      port,
...
function formatConnectReply(info: GatewayConnectInfo): string {
  const authLine =
    info.authLabel === "token"
      ? `Gateway Token: ${info.token ?? "(missing)"}`
      : `Gateway Password: ${info.password ?? "(missing)"}`;
  const sourceSuffix = info.urlSource ? ` (source: ${info.urlSource})` : "";
  return [
    "Pairing info:",
    "",
    ...
    `6) ${authLine}`,
    "7) Tap Connect",
    "8) Back here, run /pair approve",
    "",
    `Gateway URL: ${info.url}${sourceSuffix}`,
    `Auth: ${info.authLabel}`,
  ].join("\n");
}
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "reject") {
        ...
        const rejected = await rejectDevicePairing(pending.requestId);
        if (!rejected) {
          return { text: "Pairing request not found." };
        }
        return { text: `Rejected pairing request for device ${rejected.deviceId}.` };
      }
      ...
      const connectInfo = parseGatewayConnectInfo({
        url: urlResult.url,
        auth,
        urlSource: urlResult.source,
      });
      if (!connectInfo) {
        return { text: "Error: Unable to parse gateway URL." };
      }

      return { text: formatConnectReply(connectInfo) };
    },
  });
}
